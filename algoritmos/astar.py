...
def empaquetar_estado(posicion: Coordenada, muestras: Set[Coordenada], en_nave: bool, combustible: int) -> Estado:
    """Convierte los datos del estado en una tupla inmutable"""
    return (posicion, tuple(sorted(muestras)), en_nave, combustible)


def desempaquetar_estado(estado: Estado):
    posicion, muestras_tupla, en_nave, combustible = estado
    return posicion, set(muestras_tupla), en_nave, combustible
...
def generar_estados_vecinos(estado_actual: Estado, mapa: List[List[int]], posicion_nave: Coordenada):
    ...
    Aplica las reglas de movimiento:
      ...
      - Al llegar a la nave: recarga combustible a 20 y en_nave=True
      - Al llegar a una muestra: la elimina de las muestras restantes
    """
    posicion_actual, muestras_restantes, en_nave, combustible = desempaquetar_estado(estado_actual)
    ...
    for desplazamiento_fila, desplazamiento_columna in movimientos:
        ...
        if mapa[nueva_fila][nueva_col] == 5:
            nuevo_en_nave = True
        ...
        nuevo_estado = empaquetar_estado(nueva_posicion, nuevas_muestras, nuevo_en_nave, nuevo_combustible)
...
def busqueda_a_estrella(mapa: List[List[int]], posicion_inicio: Coordenada, posicion_nave: Coordenada, muestras_objetivo: Set[Coordenada]):
    ...
    estado_inicial = empaquetar_estado(posicion_inicio, muestras_objetivo, False, 0)
    ...
    while nodos_por_explorar:
        ...
        for estado_vecino, costo_mov in generar_estados_vecinos(estado_actual, mapa, posicion_nave):
            ...
            if estado_vecino not in costo_g or nuevo_costo_g < costo_g[estado_vecino]:
                ...
                posicion_vecina, muestras_vecinas, en_nave_vecina, combustible_vecino = desempaquetar_estado(estado_vecino)
